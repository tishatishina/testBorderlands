...
#include <vector>
    ...
    void InitGame() {
        ...
        user.emotion[0] = { min(O, 100) };
        user.emotion[1] = { min(O, 100) };
        user.emotion[2] = { min(O, 100) };
        user.emotion[3] = { min(O, 100) };
        user.emotion[4] = { min(O, 100) };
        user.emotion[5] = { min(O, 100) };
    ...
    void scan_emotion() {
                    ...
                    if (user.emotion[i] >= 100 || user.emotion[i] <= 0) {
                        ...
                        for (int j = 0; j < room[user.current_loc].portal.size(); j++) {
                            ...
                            if (room[user.current_loc].portal[j].name == "blue") {
                                ...
                                if (room[user.current_loc].name == "sadness") {

                                    //user.current_loc = 1 + rand() % 6;
                    ...
                    if (user.emotion[i] >= 100 || user.emotion[i] <= 0) {
                        ...
                        for (int j = 0; j < room[user.current_loc].portal.size(); j++) {
                            if (room[user.current_loc].portal[j].name == "red") {
                                room[user.current_loc].activ = false;
                            }
                        }
                    }
        ...
        void swap() {
           ...
           /* for (int i = 0; i < 6; i++) {
            ...
            if (user.emotion[user.current_loc] <= 0 || user.emotion[user.current_loc] >= 100) {
                for (int i = 0; i < 6; i++) {
                    if (i != user.current_loc && user.emotion[i] > 0 && user.emotion[i] < 100) {
                        user.current_loc = i;
                        cout << ">> Ты переместился в " << room[user.current_loc].name << " из-за нестабильности.\n";
                        break;
                    }
                }
            }
                ...
                for (int i = 0; i < room[user.current_loc].portal.size(); i++) {
                    ...
                    //if (p.activ) {
                        ...
                        if (chouse == p.name) {
                            ...
                            if (room[room[user.current_loc].portal[i].target].activ) {
